...
Ryan has a NocoDB instance (`nocodb.socko.us`) storing detailed electrical configuration for 2 homes. The data includes areas, panels, circuits, receptacles, and loads. He wants:
...
- **Domain:** e.g., `electrical.socko.us`
...
```
You are an electrical configuration assistant for Ryan's homes. You have access 
...
services:
  electrical-ui:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.electrical.rule=Host(`electrical.socko.us`)"
...
rest:
  - resource: "http://nocodb:8080/api/v2/meta/bases/nc2mrkpyv6f2apa8/tables/Circuits/records"
    method: GET
    headers:
...
rest_command:
  update_circuit_status:
    url: "http://nocodb:8080/api/v2/meta/bases/nc2mrkpyv6f2apa8/tables/Circuits/records/{{ record_id }}"
    method: PATCH
    headers:
      xc-auth: "YOUR_NOCODB_TOKEN"
...
const ha_ws = new WebSocket('ws://homeassistant.local:8123/api/websocket');
