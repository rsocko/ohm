...
```
┌─────────────────────────────────────────────────┐
│  📋 Adding 4 items to Ryan's Office             │
├─────────────────────────────────────────────────┤
...
|---|---|
| "Ryan's office has 4 recessed ceiling lights on a dimmer" | Create Load (Ceiling Lights, Fixture_Count=4), Create Receptacle (Dimmer Switch, controls Ceiling Lights) |
...
│  3. Ryan's Office Ceiling Lights        │
│                                         │
...
```
User: (viewing Ryan's Office) "The ceiling lights are on circuit 3"
AI: I'll assign Ryan's Office Ceiling Lights to Circuit 3.

┌─────────────────────────────────────────┐
│  🔗 Link  │ Ceiling Lights → Circuit 3  │
│  (Ryan's Office — based on your         │
│   current view)                         │
├─────────────────────────────────────────┤
│  [✓ Confirm]          [✕ Cancel]         │
└─────────────────────────────────────────┘
...
│  [You] 🎤 "Ryan's office"               │
│                                          │
│  [AI] Got it! What's in Ryan's Office?   │
│                                          │
...
**User says:** "Ryan's office has 4 recessed ceiling lights on a dimmer on the north wall, circuit 7"

### Step 1: Voice → Text
```
SpeechRecognition → "Ryan's office has 4 recessed ceiling lights on a dimmer on the north wall circuit 7"
```

### Step 2: Text → AI (with context)
```json
{
  "message": "Ryan's office has 4 recessed ceiling lights on a dimmer on the north wall circuit 7",
...
The LLM identifies:
- Area: "Ryan's Office" (search existing → found, ID=42)
...
```json
{
  "summary": "Adding ceiling lights with dimmer to Ryan's Office on Circuit 7",
  "operations": [
    { "action": "create", "table": "Load", "label": "Ryan's Office Ceiling Lights",
    ...
    { "action": "link", "table": "Receptacle", "label": "N-Wall Dimmer → Circuit 7",
      "linkTarget": { "table": "Circuit", "recordId": "77" } },
    { "action": "link", "table": "Load", "label": "Ceiling Lights → Ryan's Office",
      "linkTarget": { "table": "Area", "recordId": "42" } },
    { "action": "link", "table": "Receptacle", "label": "N-Wall Dimmer → Ryan's Office",
      "linkTarget": { "table": "Area", "recordId": "42" } }
  ]
}
```
...
5. Link Load → Area "Ryan's Office"
6. Link Receptacle → Area "Ryan's Office"
...
3. **Auto-naming convention** — Propose `{Area} {Descriptor}` (e.g., "Ryan's Office Ceiling Lights") — confirm this matches existing naming patterns in the DB.
