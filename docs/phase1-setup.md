...
```json
{
  "mcpServers": {
    "nocodb-electrical": {
      ...
      "url": "http://nocodb.socko.us/mcp/nc2mrkpyv6f2apa8",
      "headers": {
...
services:
  open-webui:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.open-webui.rule=Host(`open-webui.socko.us`)"
...
2. Set:
   ...
   - **System Prompt:**
     ```
     You are an electrical configuration assistant for Ryan's homes. You have 
